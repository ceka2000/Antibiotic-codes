...
scatter OutcomeAdjusted Time, connect(1) msize(vsmall) lcolor(black) mcolor(black) ///
	xlabel(1(12)162 168, valuelabel angle(vertical) ) ///
	title(Ciprofloxacin) ytitle(Prescriptions (per 1000 000 population)) xtitle(Month) ///
	ylabel(0(1000)7000 , angle(horizontal) nogrid) graphregion(color(white)) ///
	xline(63, lc(black) lp(dash))

** removing data points post oct 2009 due to a shortage in the drug
drop if _n > 130

//Change level and trend according to new generic entry date see Methods
//2004m3

drop Time Level Trend
gen Time=_n
gen Level=0
replace Level=1 if Time>62
gen Trend=0
replace Trend=_n-62 if Time>62
...
//Removing outlier to see how it will impact results
//replace OutcomeAdjusted=. if Time==34
...
scatter OutcomeAdjusted Time, connect(1) msize (small) ///
	xlabel(1(12)130, valuelabel angle(vertical) ) ytitle(Prescription) ///
	ylabel( , angle(horizontal) nogrid) ///
	title("", color(black)) ///
	graphregion(color(white)) xline(63 ) ///
	|| line yhat Time
...
prais OutcomeAdjusted Level pre6 post6
display 100*((427.8151)/(3920.402 - (427.8151))) 

//Predict relative change at 12 months
prais OutcomeAdjusted Level  pre12 post12
display 100*((653.2976)/(4097.488 - (653.2976)))

//Predict relative change at 18 months
prais OutcomeAdjusted Level  pre18 post18 
display 100*((878.7801)/(4274.575 - (878.7801)))

//Predict relative change at 24 months
prais OutcomeAdjusted Level  pre24 post24
display 100*((1104.263)/(4451.661 - (1104.263)))
