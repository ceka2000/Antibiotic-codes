...
scatter OutcomeAdjusted Time, connect(1) msize(vsmall) lcolor(black) mcolor(black) ///
	...
	ytitle(Clefuroxime axetil prescriptions (per 1000 000 population)) xtitle(Month) ///
	ylabel( , angle(horizontal) nogrid) graphregion(color(white)) ///
	xline(39, lc(black) lp(dash))
	
/// checking linearity
regress OutcomeAdjusted Time if Level == 0
rvfplot, yline(0) graphregion(color(white)) ///
	ylabel(, angle(horizontal) nogrid) title("Clefuroxime axetil", color(black)) 
...
rvfplot, yline(0,lc(black)) color(black) graphregion(color(white)) ///
	title(Cefuroxime axetil, color(black)) ///
...
scatter OutcomeAdjusted Time, connect(1) msize (small) ///
	...
	ylabel( , angle(horizontal) nogrid) ///
	title(Cefuroxime axetil, color(black)) ytitle(Prescriptions (per 1000 000 population)) xtitle(Month) ///
	graphregion(color(white)) xline(39) ///
	|| line yhat Time
