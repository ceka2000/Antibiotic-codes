...
scatter OutcomeAdjusted Time, connect(1) msize(vsmall) lcolor(black) mcolor(black) ///
	...
	ytitle(Demeclocycline prescriptions (per 1000 000 population)) xtitle(Month) ///
...
rvfplot, yline(0) graphregion(color(white)) ///
	ylabel(, angle(horizontal) nogrid) title("Demeclocycline", color(black)) 
...
rvfplot, yline(0,lc(black)) color(black) graphregion(color(white)) ///
	title(Demeclocycline, color(black)) ///
...
scatter OutcomeAdjusted Time, connect(1) msize (small) ///
	...
	ylabel(, angle(horizontal) nogrid) ///
	title(Demeclocycline, color(black)) ytitle(Prescriptions (per 1000 000 population)) ///
...
scatter OutcomeAdjusted Time, connect(1) msize (small) ///
	xlabel(1(12)160 168, valuelabel angle(vertical) ) ///
	ylabel(, angle(horizontal) nogrid) ///
	title(Demeclocycline, color(black)) ytitle(Prescriptions (per 1000 000 population)) xtitle(Month)  ///
